...
///////////////////////////////////////   AAAhhhhhhh..... ToDo...
		...
		//utils.LogMsg(3,"AddDepartment JSON string got: " + string(data) )
		// fmt.Println("AddDepartment JSON string got: " + data)
	//	utils.LogMsg(3,"AddDepartment JSON got: " + data.NewDepName )
		...
		defer db.Close()	
	 	...
	 	sql := `INSERT INTO appdb.Abteilungen ("name","FirmaId","notifyMail", "enabled") VALUES 
     ('` + data.NewDepName + `', '` +  strconv.Itoa(data.NewDepCompany) +`',  '` + data.NewDepMail +`',    
     '1');`
	//	sql := `TEST`
