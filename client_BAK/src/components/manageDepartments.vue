    ...
    <div class="manageDepartments">
        ...
        <!--     <h1> {{msg}} </h1> -->
          ...
          <section class="form">          
           ...
           <select name="NewDepFirma" v-model="selected" @click=updateAllData() class="btn btn-secondary dropdown-toggle" >
               <!--  <option :value="comp.id">{{comp.id}} </option> -->
               <option v-for="comp in Firmen" v-bind:key="comp.id" :value="comp.id">{{comp.name}} </option>               
           </select>
                
  <!-- <select  v-model="selected" id="deptList" class="btn btn-secondary dropdown-toggle">
        <option v-for="comp in Firmen" v-bind:value="comp.name">
            {{comp.name}}
        </option>
    </select>   -->
            ...
            <button id="adfds" type="button" class="btn btn-success" @click='AddDepartment'>Abteilung anlegen</button> 
                ...
                <table class="table  table-striped table-dark" >
                   <thead class="thead-dark">
                    ...
                    <tbody >
                    <tr v-for="dep in Abteilungen" v-bind:key="dep.id"> 
                        <template v-if="dep.enabled === 1">
                        <td >{{dep.name}}</td>
                        <td>{{dep.firma}}</td>                      
                        <td>{{dep.notifyMail}}</td>                                              
                        <td align = "right">    <button type="button" class="btn-disable" @click='toggleDepartment(dep.id, dep.enabled)'>disable</button> </td>
                        </template>
                        ...
                        <td class="disabledColumn" align = "right">    <button type="button" class="btn-enable" @click='toggleDepartment(dep.id, dep.enabled)'>Enable</button> </td>
                        </template>  
...
export default {
    ...
    methods: {
        async updateAllData() {               
            this.Firmen = await DBService.DBget("Firmen","enabled");   
        ...
        async AddDepartment() {
           //console.log(this.newDepName, this.newDepCompany, this.newDepMail)
